...
namespace InfinityCode.UltimateEditorEnhancer.SceneTools
{
    ...
    public class QuickAccessItem
    {
        ...
        private void InvokeWindow(QuickAccessWindowMode mode)
        {
            ...
            if (mode == QuickAccessWindowMode.utility) wnd.ShowUtility();
            else if (mode == QuickAccessWindowMode.tab) wnd.Show();
            else
            {
                EventModifiers m = QuickAccess.invokeItemModifiers;
                if (m == Prefs.popupWindowTabModifiers)
                {
                    wnd.Show();
                }
                else if (m == Prefs.popupWindowUtilityModifiers)
                {
                    wnd.ShowUtility();
                }
                else wnd.ShowPopup();
        ...
        public void ShowContextMenu()
        {
            ...
            if (type == QuickAccessItemType.window)
            {
                menu.Add($"Open {tooltip} as Tab" , () =>
                {
                    InvokeWindow(QuickAccessWindowMode.tab);
                });
                menu.Add($"Open {tooltip} as Utility", () =>
                {
                    InvokeWindow(QuickAccessWindowMode.utility);
                });
                menu.AddSeparator();
            }
            
            QuickAccess.ShowContextMenu(menu);
        }
