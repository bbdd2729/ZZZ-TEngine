...
namespace InfinityCode.UltimateEditorEnhancer.InspectorTools
{
    public class EmptyInspector: InspectorInjector
    {
        ...
        private static VisualElement itemContainer;
        ...
        private void InitItems(VisualElement parent)
        {
            ...
            foreach (Group g in EmptyInspectorReferences.items)
            {
                if (!g.enabled || g.count == 0) continue;

                CreateLabel(itemContainer, g.title);
                VisualElement container = CreateContainer(itemContainer);
        ...
        protected override bool OnInject(EditorWindow wnd, VisualElement mainContainer, VisualElement editorsList)
        {
            ...
            if (!Prefs.emptyInspector || EmptyInspectorReferences.count == 0) return false;
        ...
        {
            VisualElement el = itemContainer[i];
            ...
            if (hasVisible)
            {
                itemContainer[i - 1].style.display = DisplayStyle.Flex;
                el.style.display = DisplayStyle.Flex;
            }

            else
            {
                itemContainer[i - 1].style.display = DisplayStyle.None;
        ...
        private void UpdateFilteredItems()
        {
            ...
            {
                for (int i = 0; i < itemContainer.childCount; i += 2)
                {
                    itemContainer[i].style.display = DisplayStyle.Flex;
                    VisualElement container = itemContainer[i + 1];
            ...
            for (int i = 1; i < itemContainer.childCount; i += 2)
            {
