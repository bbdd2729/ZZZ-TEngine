...
namespace InfinityCode.UltimateEditorEnhancer.Windows
{
    public class GettingStarted : EditorWindow
    {
        ...
        private static TTreeViewState treeViewState2;
        ...
        private void DrawTableOfContent()
        {
            ...
            else
            {
                if (treeView2 == null)
                {
                    treeViewState2 = new TTreeViewState();
            ...
            foreach (Slide slide in flatSlides.Where(s => s.added > 0).OrderByDescending(s => s.updated).ThenBy(s => s.title))
            {
                if (slide.updated != version)
                {
                    ...
                    group = new Slide
                    {
                        ...
                        index = flatSlides.Count + versionSlides.Count + 1,
                        id = flatSlides.Count + versionSlides.Count + 1,
                    };
        ...
        internal class GSTreeView : TTreeView
        {
            private List<TTreeViewItem> allItems;
            private bool useVersionSlides; 

            public GSTreeView(TTreeViewState state, bool useVersionSlides) : base(state)
            {
            ...
            {
                TTreeViewItem root = new TTreeViewItem { id = -1, depth = -1, displayName = "Root" };
                allItems = new List<TTreeViewItem>();
            ...
            {
                TTreeViewItem item = FindItem(id, rootItem);
                SetSlide((item as GSTreeViewItem).slide);
            }
        }

        public class GSTreeViewItem : TTreeViewItem
        {
            public Slide slide;
        }
