...
namespace InfinityCode.UltimateEditorEnhancer.Windows
{
    ...
    public class TimescaleWindow : EditorWindow
    {
        ...
        private static void DrawTimeScale()
        {
            ...
            if (GUILayout.Button("2")) SetTimeScale(2f);
            if (GUILayout.Button("5")) SetTimeScale(5f);
            if (GUILayout.Button("..."))
            {
...
#else 
                ShowPopupWindow(InputManager.mousePosition + new Vector2(-20, 20));
#endif
            }
            Timer.Refresh();
        }

        private static void SetTimeScale(float v)
        {
            Time.timeScale = v;
            Timer.Refresh();
        }
