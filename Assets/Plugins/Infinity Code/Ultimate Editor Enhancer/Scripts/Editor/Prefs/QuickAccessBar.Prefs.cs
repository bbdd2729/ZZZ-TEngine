    ...
    public partial class Prefs
    {
        ...
        public class QuickAccessBarManager : StandalonePrefManager<QuickAccessBarManager>
        {
            ...
            public static JsonArray json
            {
                ...
                set
                {
                    if (QuickAccessReferences.count > 0)
                    {
                        if (!EditorUtility.DisplayDialog("Import Quick Access Items", "Quick Access Bar already contain items", "Replace", "Ignore"))
                        {
                            return;
                        }
                    }

                    QuickAccessItem[] items = value.Deserialize<QuickAccessItem[]>();
            ...
            private void AddItem(QuickAccessItemType type)
            {
                ...
                QuickAccessReferences.Add(item);
                ...
                reorderableList.list = QuickAccessReferences.items;
            }
            ...
            public override void Draw()
            {
                ...
                {
                    reorderableList = new ReorderableList(QuickAccessReferences.items, typeof(QuickAccessItem), true, true, true, true);
                ...
                {
                    foreach (QuickAccessItem item in QuickAccessReferences.items) item.expanded = false;
                    ReorderableListRef.ClearCache(reorderableList);
                }

                if (GUILayout.Button("Expand All"))
                {
                    foreach (QuickAccessItem item in QuickAccessReferences.items) item.expanded = true;
                    ReorderableListRef.ClearCache(reorderableList);
                }

                if (GUILayout.Button("Restore Default Items"))
                {
                    QuickAccessReferences.Clear();
            ...
            {
                if (index >= QuickAccessReferences.count) return;
                EditorGUI.BeginChangeCheck();
                QuickAccessItem item = QuickAccessReferences.items[index];
                ...
                if (!item.expanded)
                {
                    if (EditorGUI.EndChangeCheck()) QuickAccessReferences.Save();
                    return;
                }
            ...
            {
                QuickAccessReferences.RemoveAt(list.index);
                reorderableList.list = QuickAccessReferences.items;
            }
