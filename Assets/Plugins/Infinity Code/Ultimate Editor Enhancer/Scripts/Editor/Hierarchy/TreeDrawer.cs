...
namespace InfinityCode.UltimateEditorEnhancer.HierarchyTools
{
    ...
    {
        public static Texture2D endIcon => Resources.LoadIcon("Hierarchy_Tree_End");
        public static Texture2D lineIcon => Resources.LoadIcon("Hierarchy_Tree_Line");
        public static Texture2D middleIcon => Resources.LoadIcon("Hierarchy_Tree_Middle");

        static TreeDrawer()
        {
            HierarchyItemDrawer.Register(nameof(TreeDrawer), DrawTree, HierarchyToolOrder.Tree);
        }
        ...
        public static Color GetColor(HierarchyItem item, Transform parent)
        {
            ...
            if (!r) return GetBackground(parent.gameObject);
            Color color = GetBackground(r, parent.gameObject);
            if (parent && color == Color.gray) GetSelectionColor(parent, ref color);
            return color;
        }
