...
namespace InfinityCode.UltimateEditorEnhancer.HierarchyTools
{
    ...
    public class HeaderRule
    {
        ...
        public void Draw(HierarchyItem item, int textPadding = 0)
        {
            ...
            if (item.selected)
            {
                style = item.hovered ? headerStyleHoveredSelected : headerStyleSelected;
            }
        ...
        public bool Validate(GameObject go)
        {
            ...
            {
                HeaderCondition.nameStarts => StringHelper.StartsWith(name, value),
                HeaderCondition.nameContains => StringHelper.Contains(name, value),
                HeaderCondition.nameEqual => name == value,
                _ => false
            };
        }
    }
